...
\begin{enumerate}
	...
	\begin{equation}
		f'\left(x\right)=\sum_{n=1 \footnote{Si parte per $n=1$ perché per $n=0$ non avrei $\frac{1}{x}$ definito in $x=0$ }} ^{+\infty}na_nx^{n-1}, \ \forall x\in\left(-R,R\right)
	\end{equation}
%NOTA: il footnote all'interno dell'equazione non mi piace tantissimo perché compare una a che sembra un simbolo (tra l'altro è esattamente la stessa a di prima), però non saprei dove altro appiccicarlo
\end{enumerate}
\end{theorema}
Per dimostrare questo teorema useremo il teorema di derivabilità per serie di funzioni (teorema \ref{derivabilitatermineatermine}, pag. \pageref{derivabilitatermineatermine}, capitolo \refChapter{seriefunzioni}): poiché le ipotesi $1)$ e $2)$ sono banalmente verificate, dobbiamo contrarci sull'ipotesi $3)$, ovvero abbiamo bisogno di informazioni sulla convergenza uniforme della serie delle derivate $\displaystyle\sum_{n=1}^{+\infty}na_nx^{n-1}$; poiché la serie delle derivate è ancora una serie di potenze, allora ci basta studiare il raggio di convergenza.
...
%NOTA: non funziona il titoletto nell'esempio
\begin{example}[Controesempio funzione $\mathcal{C}^{\infty}$ ma non analitica] ~{}\\
...
\end{equation*}
che converge ma \textit{non} a $f$, ossia $f\left(x\right)\neq 	\displaystyle \sum_{k=0}^{+\infty}\frac{f^{\left(k\right)}\left(0\right)}{k!}x^k,\ \forall x\neq 0$.
\end{example}
