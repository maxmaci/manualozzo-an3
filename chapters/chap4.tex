...
\begin{introduction}
	...
	\end{flushright}
\end{introduction}
\lettrine[findent=1pt, nindent=0pt]{L}{e} Nel \refChapterOnly{convergenzafunzioni} abbiamo iniziato a trattare la convergenza uniforme e puntuale di successioni di funzioni. Adesso passiamo a parlare di serie di funzioni.
...
\begin{theorema}[Teorema di Abel.]~{}\\
	Sia data la serie di potenze $\displaystyle\sum_{n=0}^{+\infty}a_nz^n$ con raggio di convergenza $R>0$. Se $\exists z_0=Re^{i\theta_0}$ (ovvero $z_0$ sta nel bordo del raggio di convergenza) tale che $\displaystyle\sum_{n=0}^{+\infty}a_nz_0^n$ converge, allora\\
...
\begin{lemming}[Convergenza della serie di derivate della serie di potenze.]~{}\\
	Sia data la serie di potenze $\displaystyle\sum_{n=0}^{+\infty}a_nx^n$ e sia $R>0$ il suo raggio di convergenza. Allora la serie di potenze $\displaystyle\sum_{n=1}^{+\infty}na_nx^{n-1}$ ha lo stesso raggio di convergenza $R$.
\end{lemming}
\begin{demonstration}
...
\end{equation*}
Poichè $\nicefrac{n+1}{n}\to 1$ per $n\to+\infty$, possiamo applicare Cauchy-Hadamard sulla serie di potenze $\displaystyle\sum_{n=0}^{+\infty}a_nx^n$ con raggio di convergenza $R>0$: poiché
\begin{equation*}
...
\begin{example}\textsc{Controesempio funzione $\mathcal{C}^{\infty}$ ma non analitica.}\\
Consideriamo la funzione
	...
	\end{equation*}
\begin{itemize}
	\item $f$ è certamente di classe $\mathcal{A}^{\infty}\left(\realset\setminus\left\{0\right\}\right)$.
...
Utilizzando i confronti di crescita, dalla relazione appena mostrata si vede che
\begin{equation*}
...
\end{equation*}
Ricordiamo l'espressione del resto in \textit{forma di Lagrange} \index{Lagrange!resto}, che è di tipo quantitativo:
\begin{equation*}
...
\begin{gather*}
	0\leq \abs{\frac{f^{\left(n+1\right)}\left(\xi\right)}{\left(n+1\right)!}\left(x-x_0\right)^{n+1}} =\frac{\abs{f^{\left(n+1\right)}\left(\xi\right)}}{\left(n+1\right)!}\abs{x-x_0}^{n+1} \stackrel{\forall n\geq n_0}{\leq} \\ \leq \frac{\frac{M\Ccancel[red]{\left(n+1\right)!}}{r_0^{n+1}}}{\Ccancel[red]{\left(n+1\right)!}}\abs{x-x_0}^{n+1}
...
\begin{solution}
	...
	\end{equation*}
Fissiamo $x_0\in\left(-R,R\right)$ e consideriamo $r_0\coloneqq R-\abs{x_0}>0$; $\forall x\in \left(x_0-r_0,x_0+r_0\right)$, applicando la formula del binomio di Newton si ricava
\begin{equation*}
...
\begin{equation*}
	f\left(x\right)=f(x)=\sum_{m=0}^{+\infty} \sum_{n=0}^{+\infty} a_{n+m} \binom{n+m}{n} x_0^{n} (x-x_0)^m=\sum_{m=0}^{+\infty} b_m (x-x_0)^m
\end{equation*}
con
\begin{equation*}
...
e dunque
\begin{equation*}
	w=\log_{\realset}\abs{z}+i(\arg(z)+2k\pi),\quad k\in \integerset
\end{equation*}
\end{demonstration}
\begin{observe}
...
\begin{exercise}
	...
	\begin{enumerate}[label=\alph*]
		\item $\log 2 + i\left(-\dfrac{\pi}{2}+2k\pi\right),\ k\in\integerset$
		\item $-\log 2 + i\left(\dfrac{\pi}{2}+2k\pi\right),\ k\in\integerset$
		\item $\log 2 + i\left(\dfrac{\pi}{2}+2k\pi\right),\ k\in\integerset$
		\item $-\log 2 + i\left(-\dfrac{\pi}{2}+2k\pi\right),\ k\in\integerset$
	\end{enumerate}
...
\begin{equation*}
	\log_{\complexset}\left(-2i\right)=\log_{\realset}\abs{-2i}+i\left(\arg\left(-2i\right)+2k\pi\right)=\log 2+i\left(-\frac{\pi}{2}+2k\pi\right),\ k\in\integerset
\end{equation*}
