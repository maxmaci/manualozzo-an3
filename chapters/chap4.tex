...
\begin{demonstration}
	...
	\begin{equation*}
		\displaystyle\sum_{n=0}^{+\infty}a_nz_0^n
	\end{equation*}
...
\begin{define}[Cerchio e raggio di convergenza.]~{}\\
	...
	\end{equation*}
	l'insieme di convergenza della serie di potenze centrata in $z_0=0$ e consideriamo l'insieme $E=\left\{\abs{z}\mid z\in A\right\}\subseteq\realset$ dato da tutti i moduli dei punti di convergenza della serie. Il \textbf{raggio di convergenza}\index{raggio di convergenza} è definito come
	\begin{equation*}
...
\begin{theorema}[Converge uniforme delle serie di potenze.]~{}\\\label{convergenzasottoinsiemeH}
	...
	\begin{enumerate} % TO DO: controllare se H deve essere relativamente compatto come dice Zucco, questo è quello scritto da Walter
	...
	\end{enumerate}
\end{theorema}
\begin{demonstration} Per questa dimostrazione useremo il \textit{criterio di Weierstrass} enunciato nel \refChapterOnly{seriefunzioni}, sezione \ref{criteriodiweierstrass}, pag. \pageref{criteriodiweierstrass}.
	\begin{enumerate}
		...
		\end{equation*}
		converge per ipotesi sulla convergenza sul bordo.
	\end{enumerate}
\end{demonstration}
\begin{example}\textsc{Serie geometrica.}\\
	Sulla \textit{serie geometrica}
	\begin{equation*}
		\sum_{n=0}^{+\infty}z^n
	\end{equation*}
...
\end{observe}
Per dimostrare questo teorema useremo il teorema di derivabilità per serie di funzioni (\refChapterOnly{seriefunzioni}, teorema \ref{derivabilitatermineatermine}, pag. \pageref{derivabilitatermineatermine}): poiché le ipotesi di tale teorema $1)$ e $2)$ sono banalmente verificate, dobbiamo contrarci sull'ipotesi $3)$, ovvero abbiamo bisogno di informazioni sulla convergenza uniforme della serie delle derivate
\begin{equation*}
...
\end{equation*}% TO DO: sostituire nei dot qualcosa?
cioè $R'=R$.
\end{demonstration}
...
\begin{theorema}[Analiticità di {$e^x$}, {$\cos x$}, {$\sin x$}, {$\left(1+x\right)^\alpha$}.]~{}\\% TO DO: allinearli (chiedere ad Elisa come li voleva, se li voleva)
