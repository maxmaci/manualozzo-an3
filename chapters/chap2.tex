...
\begin{theorema}[Teorema di integrabilità per successioni, passaggio al limite sotto segno di integrale.]~{}\\\label{passaggioallimitecontinuitàuniforme}
	Siano $\funz{f_n,f}{\left[a,b\right]}{\realset},\ n\geq 1$ tali che
	\begin{enumerate}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\lim_{n\to+\infty}\int_{a}^{b}f_n\left(x\right)dx=\int_{a}^{b}\lim_{n\to+\infty}f_n\left(x\right)dx=\int_{a}^{b}f\left(x\right)dx
		\end{equation}
	\end{enumerate}
\end{theorema}
\begin{observe}
