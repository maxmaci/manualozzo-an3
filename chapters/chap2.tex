...
%ALTERNATIVA
%		\sup_{x\in \left[0,1\right]}\frac{x^n}{n}=\frac{x^n}{n}_{|_{x=1}}\,=\frac{1}{n}
	\end{equation*}
	allora
	\begin{equation*}
...
%NOTA Dambrosioi ha specificato che per avere Cauchy lo spazio deve essere completo, quindi non è una condizione da mettere fra parentesi
	\begin{equation}
...
%NOTA non è A che deve essere finito, ma il sup. Al massimo A deve essere compatto per ammettere massimo per Weierstrass
	\item Se è finito, allora $\displaystyle\sup_{x\in A}N\left(\epsilon\right)=\max_{x\in A}N\left(\epsilon\right)=N\left(\epsilon\right)$ e c'è convergenza uniforme.
...
\begin{equation*}
	\abs{f\left(x\right)}=\abs{f\left(x\right)+f_{N_2}(x)-f_{N_2}(x)}\leq\abs{f\left(x\right)-f_{N_2}(x)}+\abs{f_{N_2}(x)}\leq 2+M_2=M,\ \forall x\in A
\end{equation*}
\end{demonstration}
\begin{digression}
...
\end{observe}
%NOTA: personalmente metterei tutto in un unico esempio in cui enunciamo in un enumerate i titoletti e poi facciamo la dimostrazione con l'enumerate, anche ripetendo il numeretto volendo. Così è tutto più compatto quando si fa una sfogliata apposta per vedere quali tipi di controesempi ci sono e quali proprietà non valgono
...
\begin{example}{\textsc{La convergenza uniforme \textit{non} è sufficiente per trasferire alla funzione limite l'integrabilità su un intervallo \textit{illimitato} } } ~{} \\
...
\end{example}
\begin{example}{\textsc{La convergenza uniforme \textit{non} è condizione necessaria per il passaggio al limite sotto segno di integrale.} } ~{}\\
...
\end{example}
%OLD : c'è una ripetizione, l'ho lasciato per sicurezza
%\begin{example}
%	Consideriamo la successione di funzioni $f_n\left(x\right)=x^n$ definite su $\left[0,1\right]$. Osserviamo che
%	\begin{equation*}
%		\lim_{n\to+\infty}\int_{0}^{1}x^ndx=\lim_{n\to+\infty}\left[\frac{1}{n+1}x^{n+1}\right]_{0}^{1}=\lim_{n\to+\infty}\frac{1}{n+1}=0
...
%	$x^n$ non converge uniformemente su $\left[0,1\right]$, ma il passaggio al limite sotto segno di integrale si verifica comunque.
%\end{example}
\begin{example}{\textsc{La convergenza uniforme \textit{non} è condizione sufficiente per il passaggio al limite sotto il segno di integrale nel caso in un intervallo \textit{illimitato}. } } ~{} \\
...
\end{example}
\begin{example}{\textsc{La convergenza puntuale \textit{non} è condizione sufficiente per il passaggio al limite sotto il segno di integrale, nemmeno nel caso di un intervallo limitato.} } ~{}\\
...
%NOTA: sicuro che sia un'esponenziale e non una potenza?
	\begin{equation*}
...
\begin{example}{\textsc{La convergenza uniforme \textit{non} è condizione sufficiente per trasferire alla funzione limite la derivabilità.} }~{}\\
	...
	\end{itemize}
\end{example}
\begin{example}{\textsc{La convergenza uniforme \textit{non} è condizione sufficiente per poter scambiare limite e derivata, anche se si aggiunge l'ipotesi che la funzione limite sia derivabile.} } ~{}\\
...
\begin{equation}
	\forall u,v\in\left(\alpha,\beta\right),\  \abs{h\left(u\right)-h\left(v\right)}\leq\left(\sup_{x\in\left(\alpha,\beta\right)}\abs{h'\left(x\right)}\right)\abs{u-v}
\end{equation}
\end{corollary}
