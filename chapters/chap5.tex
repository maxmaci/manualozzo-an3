...
%Peano ha lavorato a Torino, però essendo di origini cuneesi potrebbe essere frainteso come "orginario di Torino", e i cuneesi si arrabbiano (sì, i cioccolatini)
...
%ho sostituito \mid con \middle| e inserito i \ prima e dopo perché a pagina 92 c'era già \middle| ma era tutto attaccato		
%un'alternativa usata nel chap4 riga 79 è \ \middle\vert \
		\end{equation}
...
%stesso discorso del commento precedente
		\end{equation}
	\end{itemize}
...
%fastidiosamente c'è un punto a capo da solo
	...
	\end{define}
%NEW: riformulazione
	Per qualsiasi insieme test $E$ insiemisticamente si ha che $E=(E\cap A) \cup \left(E\cap A^C\right)$. Si impone che $A$ è misurabile se per qualsiasi \textbf{insieme test}\index{insieme!test} arbitrario (non necessariamente misurabile!) la misura $m_n^X$ è additiva su questa decomposizione
%OLD
...
\begin{examplewt}[Misura dei razionali]
	Posto $n=1$ (ossia consideriamo la misura in $\realset$) e sapendo che i razionali $\rationalset$ sono un insieme numerabile, abbiamo che la loro misura è nulla:
	\begin{equation*}
...
\end{theorema}
\begin{attention}
	Nel teorema abbiamo usato la misura esterna $m_n^X$ perché non sappiamo ancora che gli insiemi sono misurabili.
\end{attention}
\begin{demonstration}
...
È già noto che non tutti gli insiemi misurabili secondo Lebesgue lo sono secondo Peano-Jordan: i primi sono un'estensione dei secondi.
\begin{example}
...
\begin{demonstration}
	...
	\end{itemize}
Sappiamo che $E$ è misurabile secondo Peano-Jordan, dunque per il criterio equivalente possiamo approssimarlo dall'interno e dall'esterno con unioni finite di insiemi elementari: $\forall \epsilon >0,\ \exists A_{\epsilon}, B_{\epsilon}$ unioni finite di parallelepipedi con $B_{\epsilon}\subseteq E\subseteq A_{\epsilon}$ tali che $m_{PJ}\left(A_{\epsilon}\setminus B_{\epsilon}\right)<\epsilon$, cioé la misura dell' "intercapedine" fra i due insiemi è arbitrariamente piccola; siccome il bordo è ininfluente posso considerare $A_\epsilon$ aperto. Allora l'insieme $A_{\epsilon}$ così definito è proprio quello che stavamo cercando grazie alla seconda condizione. Noto innanzitutto che $A_{\epsilon}\setminus E\subseteq A_{\epsilon}\setminus B_{\epsilon}$, per monotonia della misura esterna otteniamo:
\begin{equation*}
	m_{n}^{X}\left(A_{\epsilon}\setminus E\right) \leq	m_{n}^{X}\left(A_{\epsilon}\setminus B_{\epsilon}\right)=m_{PJ}^X\left(A_{\epsilon}\setminus B_{\epsilon}\right) \stackrel{!}{=} m_{PJ}\left(A_{\epsilon}\setminus B_{\epsilon}\right)<\epsilon
\end{equation*}
dove l'uguaglianza (!) è dovuta al fatto che le misure di Lebesgue e Peano-Jordan coincidono su insiemi elementari.
\end{demonstration}
...
\begin{define}[Misura e spazio di misura]
	Dato $\left(X,\mathcal{M}\right)$ uno spazio misurabile \footnote{ricordiamo che $\mathcal{M}$ è una $\sigma$-algebra}, una funzione $\funz{\mu}{\mathcal{M}}{\realset^{\ast}=\left[-\infty,+\infty\right]}$ è detta \textbf{misura}\index{misura} se soddisfa le seguenti proprietà:
	\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item $\mu(X)=+\infty$.
			\item $\displaystyle X=\bigcup_{n\geq 1}X_n$, con $X_n\in\mathcal{M}$ tale che $\mu\left(X_n\right) < +\infty$, cioé lo posso ricostruire come unione numerabile di insiemi a misura finita.
		\end{itemize}
...
%		\begin{tabular}[c]{@{}c@{}}Topologia\\ (famiglia degli aperti)\end{tabular}     & $\topo$                            & $\mathfrak{c}$    
	\end{tabular}
\end{center}
%nutro qualche dubbio sull'inserire così la topologia, specialmente riguardo la sua cardinalità
%	stiamo sempre parlando di topologia sui reali? è quella euclidea? messa così confonde un po'
...
\begin{itemize}
	...
	\begin{equation*}
		...
		\end{cases}
	\end{equation*}
	dove l'uguaglianza (!) è dovuta alla $\sigma$-additività di $m_1$ e invece (!!) è dovuta all'invarianza per traslazione di $m_1$, infatti $V=V_n + q_n$
