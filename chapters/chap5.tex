...
\begin{define}[Parallelepipedo {$n$}-dimensionale.]~{}\\
	Un \textbf{parallelepipedo}\index{parallelepipedo} $n$-dimensionale è un \textit{plurintervallo}, ossia come il prodotto cartesiano di $n$ intervalli:
	\begin{equation}
	...
	\begin{define}[Misura di Peano-Jordan.]~{}\\
		Un insieme limitato $A$ è \textbf{misurabile secondo Peano-Jordan}\index{misura!secondo Peano-Jordan} se $m_{PJ}^X\left(A\right)=m_{PJ,X}\left(A\right)$ e la \textbf{misura} (secondo P-J) dell'insieme è
		\begin{equation}
	...
	\begin{define}[Parallelepipedo {$n$}-dimensionale.]~{}\\
		Un \textbf{parallelepipedo}\index{parallelepipedo} $n$-dimensionale è un \textit{plurintervallo}, ossia come il prodotto cartesiano di $n$ intervalli eventualmente \textit{degeneri}:
		\begin{equation}
	...
	\begin{itemize}
		...
		\end{equation}
	\end{itemize}
	Osserviamo che per $m^X$ vale \textit{solo} la  $\sigma$-\textit{subadditività}, ma non la $\sigma$-additività.
...
\begin{example}
	...
	\end{equation*}
\end{example}
Esistono anche insiemi di misura nulla con \textit{cardinalità del continuo}.
...
\begin{theorema}[Equivalenza della misura di Peano-Jordan e Lebesgue.]~{}\\
...
\begin{define}[Misura e spazio di misura.]~{}\\
	...
	\end{itemize}
\end{define}
\begin{examples}\textsc{Spazi di misura.}
	\begin{enumerate}
		...
		\end{equation*}
		\item Fissato $x_0\in X$ insieme qualunque, $\left(X,\setpart{X}\right)$ è spazio di misura con la funzione $\delta$ \textbf{di Dirac concentrata in} $x_0$\index{Delta di Dirac}:
		\begin{equation}
	...
	\end{equation}
Se $\displaystyle\sum_{n\colon x_n\in E}p_n=1$, $\mu_p$ è una \textbf{misura di probabilità discreta}\index{misura!di probabilità!discreta}, come la m.d.p. \textit{binomiale}, di \textit{Poisson}, ecc...\\
...
\end{equation}
	\end{enumerate}
\end{examples}
\subsection{Famiglie di insiemi nella teoria della misura e relazioni tra di loro}
