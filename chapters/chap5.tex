...
\end{proposition}
\begin{demonstration}
	Poiché $f\in\mathcal{L}^{1}\left(\mu\right)$, allora $\Re f$ e $\Im f$ sono misurabili; ne consegue che $\left(\Re f\right)^{\pm},\ \left(\Im f\right)^{\pm}$ sono misurabili\footnote{Si vedano le ‘‘Note aggiuntive'', a pag. \pageref{misuraparterealeimm}.}, reali e non negative. Si noti che
	\begin{align*}
		\abs{\Re f}&=\left(\Re f\right)^{+}+\left(\Re f\right)^{-}\geq 0\implies\abs{\Re f}\geq \left(\Re f\right)^{\pm}\\
		\abs{\Im f}&=\left(\Im f\right)^{+}+\left(\Im f\right)^{-}\geq 0\implies\abs{\Im f}\geq \left(\Im f\right)^{\pm}
	\end{align*}
	e che, per definizione di modulo complesso,
	\begin{align*}
		\abs{f}\geq \abs{\Re f}&&
		\abs{f}\geq \abs{\Im f}
	\end{align*}
	Pertanto, per monotonia delle integrande si osserva che
	\begin{equation}
		0\leq\int_{X}\left(\Re f\right)^{\pm}d\mu\leq \int_{X}\abs{\Re f}d\mu\leq \int_{X}\abs{f}d\mu<+\infty
	\end{equation}
	dove l'ultima disuguaglianza vale in quanto $f\in\mathcal{L}^{1}\left(\mu\right)$; il ragionamento è analogo per $\left(\Im f\right)^{\pm}$.
	La seconda ipotesi per l'integrabilità è dunque soddisfatta per tutte queste funzioni e quindi vale la tesi.
\end{demonstration}
