...
\begin{observe}~{}
	...
	\end{itemize}
\end{observe}
\begin{example}\textsc{Lemma di Fatou con disuguaglianza stretta.}\\
...
\end{observe}
\subsection{Misura di probabilità} % TO DO: controllare che non stia dicendo una cazzata, è questa la definizione di misura di probabilità?
...
\begin{theorema}[Integrale rispetto alla misura di probabilità.]~{}\\
...
\begin{equation*}
	\int_Xgd\mu_f=\int_X\left(gf\right)d\mu
\end{equation*}
	\end{enumerate}
\end{demonstration}
\begin{example}
	Riprendiamo l'esempio a pag. \pageref{gaussiana}; la funzione gaussiana
	\begin{equation*}
		f\left(x\right)=\frac{1}{\sqrt{2\pi}}e^{-\frac{x^2}{2}},\ \forall x\in X
	\end{equation*}
	in $\left(\realset,\mathcal{L}\left(\realset\right),m_1\right)$ implica la misura di probabilità ($\mu_f\left(X\right)=1$) normale
	\begin{equation*}
...
\end{equation*}
Questo si esprime dice che $\mu_f$ è \textbf{assolutamente continua rispetto a }$\mu$ e si indica $\mu_f \ll \mu$.
\end{observe}
\begin{define}[Continuità assoluta.]~{}\\
...
\begin{theorema}[Caratterizzazione delle misure ass. cont. finite.]~{}\\
...
Allora
\begin{equation}
	\lambda\ll\mu\iff\forall \epsilon>0\exists\delta>0\colon\forall E\in\mathcal{M}\colon \mu\left(E\right)<\delta\implies \lambda\left(E\right)<\epsilon
\end{equation}
\end{theorema}
...
\end{equation*}
La risposta è \textit{no}, come si può vedere dal teorema seguente.
\begin{theorema}[Teorema di Radon-Nicodym.]~{}\\
...
Tuttavia, a differenza del passo 2, dove l'integrale può essere assumere valori in $\left[0,+\infty\right]$, l'insieme dei complessi $\complexset$ non contempla il valore $+\infty$; inoltre, come vedremo, la costruzione dell'integrale scelta può presentare delle forme di indecisione che non possiamo risolvere.\\
Per proseguire, dobbiamo considerare una classe particolare di funzioni misurabili, le \textbf{funzioni integrabili}\index{funzione!integrabile}\seeonlyindex{funzione!integrabile}{integrabilità}.
...
\end{define}
\begin{observe}% TO DO: rendere seconda osservazione un attenzione
	\begin{itemize}
		\item Per definizione $f\in\mathcal{L}^{1}\left(\mu\right)\iff\abs{f}\in\mathcal{L}^{1}\left(\mu\right)$.
		\item Nel caso particolare $\funz{f}{X}{\left[0,+\infty\right)}$, se $f$ è misurabile, allora esiste
		\begin{equation*}
			\int_Xfd\mu
		\end{equation*}
...
\end{observe}
\begin{proposition}[Proprietà dell'integrale di Lebesgue per funzioni a valori complessi.]~{}
	\begin{enumerate}
...
Si ha quindi
\begin{equation*}
	\forall \epsilon > 0,\ \exists \delta >0\colon \exists E\in\mathcal{M},\ \mu\left(E\right)<\delta\implies \abs{\int_Efd\mu}\underset{\substack{\text{prop. }2\\\text{dell'int.}}}{\leq}\int_E\abs{f}d\mu<\epsilon\implies \abs{\int_Efd\mu}<\epsilon
\end{equation*}
\end{demonstration}