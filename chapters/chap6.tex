...
\end{observe}
\subsection{Misura indotta dall'integrale di Lebesgue}
...
\begin{theorema}[Integrale proprio di Riemann implica integrale di Lebesgue.]~{}\\
...
e
\begin{equation}
	\int_{\left[a,b\right]}\abs{f}dm_1=\int_{a}^{b}f\left(x\right)dx
\end{equation}
\end{theorema}
...
\begin{theorema}[Integrale improprio di Riemann e integrale di Lebesgue.]~{}\\\label{integraleimproprioriemannlebesgue}
	...
	\begin{enumerate}
		...
		\begin{equation}
			\int_{\left[a,+\infty\right)}fdm_1=\int_{a}^{+\infty}f\left(x\right)dx\in\realset
		\end{equation}
	\end{enumerate}
\end{theorema}
\begin{observe}
...
\begin{theorema}[Caratterizzazione delle funzioni integrabili secondo Riemann.]~{}\\
...
\end{example}
\begin{proposition}[Ruolo degli insiemi di misura nulla nell'integrazione.] % TO DO: proprietà
Sia $\left(X,\mathcal{M},\mu\right)$ uno spazio di misura. Allora
\begin{enumerate}
	\item Se $\funz{f}{X}{\complexset}$, $f\in\mathcal{L}^{1}\left(\mu\right)$ allora
	\begin{equation}
		\forall E\in\mathcal{M},\ \mu\left(E\right)=0\implies \int_Efd\mu=0
	\end{equation}
\item Se $\funz{f,g}{X}{\complexset}$, $f,g\in\mathcal{L}^{1}\left(\mu\right)$ allora
\begin{equation}
...
\begin{demonstration}~{}
	...
	\begin{align*}
		0=&\int_Xfd\mu\leq&\\
		 &\leq\int_{E_n}fd\mu\leq&\text{(monotonia dell'integrale rispetto al dominio)}\\
...
\begin{theorema}[Scambio tra integrale e serie per funzioni integrabili.]~{}\\
	Sia $\left(X,\mathcal{M},\mu\right)$ unop spazio di misura. Siano $\funz{f_n}{X}{\complexset}$ integrabili. Supponiamo che
	\begin{equation*}
		\sum_{n=1}^{+\infty}\int_X\abs{f_n}d\mu<+\infty
	\end{equation*}
Allora
\begin{enumerate}
	\item $\displaystyle f\left(x\right)=\sum_{n=1}^{+\infty}f_n\left(x\right)$ è definita q.o. in $X$.
...
\begin{equation}
	\mathcal{L}^{1}=\left\{\funz{f}{X}{\complexset}\text{ misurabili}\mid \int_X\abs{f}d\mu<+\infty\right\}
\end{equation}
...
\begin{enumerate}
	\item[1.] $\displaystyle f=0\implies \int_X\abs{f}d\mu=0$ ma $\displaystyle \int_X\abs{f}d\mu=0\implies f$ q.o. in $X$.
\end{enumerate}
...
\begin{equation}
	f,g\in\mathcal{L}^{1}\left(\mu\right)\colon f\sim g\iff f=g\text{ q.o. in }X
\end{equation}
...
\begin{equation}
	\funztot{\lvert\cdot\rvert}{L^{1}\left(\mu\right)}{\left[0,+\infty\right)}{\left[f\right]}{\lvert\left[f\right]\rvert=\int_X\abs{f}d\mu}
\end{equation}
Questa norma è ben posta come funzione in $L^1\left(\mu\right)$ in quanto \textit{non} dipende dal \textit{rappresentante} scelto:
\begin{equation*}
	g\in\left[f\right]\iff f=g\text{ q.o. in }X\implies \int_X\abs{g}d\mu=\int_X\abs{f}d\mu
\end{equation*}
